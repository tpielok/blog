...
$$p(\boldsymbol{\theta}\vert \boldsymbol{X}) = \frac{p(\boldsymbol{X} \vert \boldsymbol{\theta})p(\boldsymbol{\theta})}{p(\boldsymbol{X})}$$

with smooth likelihood $$p(\boldsymbol{X}\vert \boldsymbol{\theta})$$, smooth prior $$p(\boldsymbol{\theta})$$ and marginal likelihood $$p(\boldsymbol{X})$$ which is in general intractable. This is, e.g., the case for most Bayesian neural networks. 
